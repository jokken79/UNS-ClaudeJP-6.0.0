#!/usr/bin/env python3
"""Fix photo data by removing garbage bytes before JPEG marker"""
import sys
...
for candidate in candidates:
    try:
        ...
        if not photo.startswith('data:image'):
            print(f'Skipping candidate {candidate.id}: Invalid format')
            continue

        # Split prefix and base64 data
        parts = photo.split(',', 1)
        if len(parts) != 2:
            print(f'Skipping candidate {candidate.id}: No comma separator')
            continue
        ...
        if jpeg_start < 0:
            ...
            if png_start >= 0:
                if png_start > 0:
                    print(f'Candidate {candidate.id}: Removing {png_start} garbage bytes before PNG')
                    clean_data = decoded[png_start:]
                    clean_b64 = base64.b64encode(clean_data).decode('ascii')
                    candidate.photo_data_url = f'{prefix},{clean_b64}'
                    fixed_count += 1
                continue

            print(f'WARNING: Candidate {candidate.id}: No image marker found')
            continue

        # If garbage bytes exist before JPEG
        if jpeg_start > 0:
            print(f'Candidate {candidate.id}: Removing {jpeg_start} garbage bytes')

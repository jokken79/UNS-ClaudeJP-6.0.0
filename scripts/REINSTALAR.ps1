# ════════════════════════════════════════════════════════════════════════════
#  UNS-ClaudeJP 5.4 - Reinstalación Completa (PowerShell)
#  Versión: 2025-11-12 (PowerShell Edition)
#
#  Uso:
...
Write-Header "[FASE 1/3] Diagnóstico del Sistema"
...
if ($HasErrors) {
    Write-Header "[X] DIAGNÓSTICO FALLIDO"
...
Write-Status "Diagnóstico completado" "OK"

# ════════════════════════════════════════════════════════════════════════════
#  FASE 2: CONFIRMACIÓN
# ════════════════════════════════════════════════════════════════════════════

Write-Header "[FASE 2/3] Confirmación"
Write-Host "╔══════════════════════════════════════════════════════════════════════╗" -ForegroundColor $Colors.Warning
Write-Host "║                   ! ADVERTENCIA IMPORTANTE                          ║" -ForegroundColor $Colors.Warning
Write-Host "╠══════════════════════════════════════════════════════════════════════╣" -ForegroundColor $Colors.Warning
Write-Host "║ Esta acción eliminará TODOS los datos existentes:                  ║" -ForegroundColor $Colors.Warning
...
Write-Host "║   • Volúmenes Docker                                                ║" -ForegroundColor $Colors.Warning
Write-Host "║                                                                     ║" -ForegroundColor $Colors.Warning
Write-Host "║ Se creará una instalación completamente nueva.                      ║" -ForegroundColor $Colors.Warning
Write-Host "╚══════════════════════════════════════════════════════════════════════╝" -ForegroundColor $Colors.Warning
Write-Host ""

$Confirm = Read-Host "¿Continuar con la reinstalación? (S/N)"
if ($Confirm -notin @("S", "SI", "s", "si")) {
    Write-Host ""
    Write-Host "[X] Reinstalación cancelada" -ForegroundColor $Colors.Error
    Write-Host ""
    Write-Host "════════════════════════════════════════════════════════════════════"
    Write-Host "  Presiona ENTER para cerrar" -ForegroundColor $Colors.Error
    Write-Host "════════════════════════════════════════════════════════════════════"
...
Write-Header "[FASE 3/3] Reinstalación"

# Paso 1: Generar .env
Write-Header "[1/6] GENERACIÓN DE ARCHIVO DE CONFIGURACIÓN (.env)"
...
if (-not (Test-Path ".env")) {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "   i Ubicación: $(Get-Location)\.env"
        } else {
            Write-Status "ERROR: Falló la generación del archivo .env" "Error"
    ...
    Write-Status "Archivo .env ya existe (se usará el actual)" "OK"
...
try {
    ...
    Write-Host "   ! Hubo errores al detener (puede ser normal si no había servicios)"
}

Write-Status "Eliminando volúmenes antiguos..." "OK"
Write-Host "   i Se creará una instalación completamente nueva"
Write-Host ""

# Paso 3: Reconstruir imágenes
Write-Header "[3/6] RECONSTRUIR IMÁGENES DOCKER"

Write-Status "Construyendo imágenes Docker (puede tardar 5-10 minutos)..." "Info"
Write-Host "   i Se compilarán: Backend (FastAPI) + Frontend (Next.js)"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host ""
        Write-Status "ERROR: Falló la construcción de imágenes" "Error"
        Read-Host "Presiona ENTER"
        exit 1
    }
    Write-Host ""
    Write-Status "Imágenes Docker construidas correctamente" "OK"
...
Write-Host ""
Write-Status "Esperando que PostgreSQL esté lista (health check - máx 90s)..." "Info"
...
while ($waitCount -lt $maxWait) {
    try {
        ...
        if ($dbStatus -eq "healthy") {
            Write-Status "PostgreSQL está lista y saludable" "OK"
    ...
    if ($waitCount -ge $maxWait) {
        Write-Host ""
        Write-Status "TIMEOUT: PostgreSQL no respondió en 90 segundos" "Error"
...
Write-Host ""
Write-Status "Esperando que backend esté listo (20 segundos)..." "Info"
...
Write-Host ""
Write-Status "Ejecutando migraciones de Alembic (incluye triggers e índices)..." "Info"
Write-Host "   i Esto aplicará TODAS las migraciones incluyendo:"
Write-Host "   i   - Tablas base (24 tablas)"
Write-Host "   i   - Trigger de sincronización de fotos"
Write-Host "   i   - Índices de búsqueda (12 índices GIN/trigram)"

try {
    & docker exec uns-claudejp-backend bash -c "cd /app && alembic upgrade head" 2>&1 | Out-Null
    if ($LASTEXITCODE -ne 0) {
        Write-Status "ERROR: Falló la ejecución de migraciones" "Error"
    ...
    Write-Host "   i Tablas + Triggers + Índices configurados"
} catch {
    Write-Status "ERROR: $_" "Error"
    Read-Host "Presiona ENTER"
    exit 1
...
try {
    ...
    Write-Status "Inicialización de base de datos completada" "OK"
} catch {
    Write-Host "   ! Warning: Error en verificación de tablas"
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Status "Sincronización completada" "OK"
        Write-Host "   i Candidatos actualizados a status 'hired' si tienen empleado asociado"
    } else {
        Write-Host "   ! Warning: Error en sincronización (puede ser normal si no hay datos)"
    }
} catch {
    Write-Host "   ! Warning: Error en sincronización (puede ser normal si no hay datos)"
}
...
Write-Host "   i Backend ya está corriendo desde paso 5"
...
Write-Host ""
Write-Status "Esperando compilación del frontend (60 segundos)..." "Info"
...
Write-Status "Compilación completada" "OK"
Write-Host ""
...
Write-Header "[OK] REINSTALACIÓN COMPLETADA EXITOSAMENTE"
...
# Limpieza de fotos
Write-Header "[PASO FINAL] LIMPIEZA AUTOMÁTICA DE FOTOS OLE"

$cleanupScript = Join-Path $ScriptDir "LIMPIAR_FOTOS_OLE.bat"
if (Test-Path $cleanupScript) {
    Write-Status "Ejecutando LIMPIAR_FOTOS_OLE.bat automáticamente..." "Info"
    try {
...
Write-Host ""
Write-Header "REINSTALACIÓN + LIMPIEZA COMPLETADA AL 100%"
